...
let entitlementActive = true;
let capturedCheckoutForm = null;

globalThis.fetch = async (input, init = {}) => {
  const url = new URL(typeof input === "string" ? input : input.url);
  const headers = new Headers(init.headers || (input instanceof Request ? input.headers : undefined));

  if (url.hostname === "api.stripe.com" && url.pathname.startsWith("/v1/prices/")) {
    const priceId = decodeURIComponent(url.pathname.slice("/v1/prices/".length));
    const monthlyId = `price_${"m".repeat(24)}`;
    const annualId = `price_${"a".repeat(24)}`;
    if (priceId === monthlyId) {
      return new Response(JSON.stringify({
        ...
        product: { id: "prod_monthly", active: true },
    ...
    if (priceId === annualId) {
      return new Response(JSON.stringify({
        ...
        product: { id: "prod_annual", active: true },
      }), { status: 200, headers: { "Content-Type": "application/json" } });
    }
  ...
  if (url.pathname === "/rest/v1/entitlements") {
    return new Response(JSON.stringify(entitlementActive ? [{ active: true, expires_at: null }] : []), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  }

  if (url.pathname === "/rest/v1/billing_customers" && (init.method || "GET").toUpperCase() === "GET") {
    return new Response(JSON.stringify([{
      user_id: userA.id,
      stripe_customer_id: "cus_live_production_smoke_001",
    }]), { status: 200, headers: { "Content-Type": "application/json" } });
  }
...
try {
  ...
  assert.equal(healthPayload.version, "6.8.2-staging-v1");
  ...
  assert.equal((await testKeyResponse.json()).error, "stripe_mode_key_required");
  ...
  assert.equal(transitionWebhookGet.status, 405);

  const transitionRoot = await worker.fetch(
    new Request("https://launch.botdhockey.com/"),
    env,
    {},
  );
