...
-- B.O.T.D. Hockey Playbook Studio production billing schema
...
-- Production safety boundary: reject Stripe test-mode records even if a
-- misconfigured backend attempts to write them.
alter table public.subscriptions
  alter column livemode set default true;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'subscriptions_production_only'
      and conrelid = 'public.subscriptions'::regclass
  ) then
    alter table public.subscriptions
      add constraint subscriptions_production_only
      check (livemode = true);
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'webhook_events_production_only'
      and conrelid = 'public.processed_webhook_events'::regclass
  ) then
    alter table public.processed_webhook_events
      add constraint webhook_events_production_only
      check (livemode = true);
  end if;
end
$$;
...
begin
  ...
  if p_livemode is not true then
    raise exception 'production refuses non-live subscription state';
  end if;
