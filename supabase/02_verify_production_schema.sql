-- B.O.T.D. production schema verification.
...
from pg_constraint
where conname in (
  'subscriptions_production_only',
  'webhook_events_production_only'
)
order by conname;
...
where table_schema = 'public'
  ...
  and table_name in ('subscriptions', 'processed_webhook_events')
order by table_name;
...
select
  ...
  (select count(*) from public.processed_webhook_events) as processed_webhook_events;
